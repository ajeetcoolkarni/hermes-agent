...
"\n"
"**CRITICAL — Workspace collision rule (prevents build corruption):**\n"
"If two or more tasks share the same `workspace_path` (or same `workspace_kind=dir` "
"target), they MUST be chained with `parents=[previous_task_id]` — never siblings "
"with no parent link. This applies regardless of whether they have the same or different "
"assignees. The dispatcher claims tasks independently by profile, not by file lock. "
"Two sibling tasks on the same workspace will spawn simultaneously, race each other's "
"git state / npm install / build output, and corrupt the workspace.\n"
"\n"
"Example proper pipeline for a shared workspace:\n"
"  1. asset-builder creates assets on workspace /path/to/project\n"
"  2. worker implements gameplay with parents=[asset-builder_task_id]\n"
"  3. reviewer QA with parents=[worker_task_id]\n"
"\n"
"Only parallel siblings when: different workspaces (or read-only shared access).\n"
"Before creating tasks, verify your graph: list every task's workspace_path and confirm "
"no two tasks target the same directory without parent-child ordering. If found, redesign.\n"
    "\n"
    "**CRITICAL — Reviewer remediation rules:**\n"
"If you are a reviewer and find failures, create a remediation task with:\n"
"- `parents=[reviewed_task_id]` ONLY — never parent-link to a blocked/archived task\n"
"- If the reviewed task is `done` (even with bad results), remediation becomes `ready` immediately\n"
"- NEVER link the docs task to a remediation task — docs stays at the end of the original chain\n"
"- Example: reviewer checks t_worker → finds bugs → creates t_remediation with parents=[t_worker]\n"
"\n"
"**CRITICAL — Docs task rules:**\n"
"Docs is ALWAYS the LAST task. It has one parent: reviewer. It NEVER parents any other task.\n"
"If docs is blocked, the pipeline stalls forever.\n"
"\n"
"## Do NOT\n"
    "\n"
