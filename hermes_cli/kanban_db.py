...
# A running task's claim is valid by default; after expiry the next
# dispatcher tick reclaims it. Workers that outlive this window should call
# ``heartbeat_claim(task_id)`` periodically. Use ``HERMES_KANBAN_CLAIM_TTL_SECONDS``
# to raise the default claim window for long single-call MCP workflows.
...
def recompute_ready(
    ...
    Archived parents also satisfy the dependency check.
    ...
    with write_txn(conn):
        ...
        waiting_rows = conn.execute(
            "SELECT id, status, workspace_path, assignee, "
        ...
        for row in waiting_rows:
            ...
            # --- sticky block check (worker-initiated kanban_block) ---
            ...
            if not all(p["status"] in ("done", "archived") for p in parents):
                continue
            # --- workspace collision check (custom: per-project serialisation) ---
            ...
            if ws_key and ws_key in busy:
                ...
                continue
            # --- assignee cap check (custom: concurrency control) ---
            assignee = row["assignee"]
            if assignee and assignee not in _LOCAL_PROFILES:
                ...
                if running_cnt >= _MAX_CONCURRENT_FALLBACK:
                    ...
                    continue
            # --- failure limit check (main: circuit breaker guard) ---
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # --- workspace guard: skip if same project already occupied (custom) ---
        ws_key = _normalize_workspace(row["workspace_path"])
        if ws_key and ws_key in busy_ws:
            result.skipped_workspace_busy.append(row["id"])
            continue
        # --- cap guard: non-local profiles max 2 concurrent (custom) ---
        if row_assignee not in _LOCAL_PROFILES:
            _cnt = _running_by_assignee.setdefault(
                row_assignee,
                _assignee_running_count(conn, row_assignee),
            )
            if _cnt >= _MAX_CONCURRENT_FALLBACK:
                result.skipped_cap.append(row["id"])
                continue
            _running_by_assignee[row_assignee] = _cnt + 1
